...
t_page Touch_SetupCompressorLinker(unsigned char matrix, t_page page)
{
  ...
  switch(matrix)
  {
    ...
    case 0x21:  
      if(!touch){on=1;
      LCD_WriteValue3(16,7, druckMax);  touch=5;} 
      break;

    // max
    case 0x31:  
      if(!touch){on=0;
      LCD_WriteValue3(11,7, druckMin);  touch=5;} 
      break;
...
t_page Touch_DataLinker(unsigned char matrix, t_page page)
{
  switch(matrix)
  {
    // auto
    case 0x21:  
      LCD_Write_TextButton(9, 0, Auto, 0);
      return DataAuto;

    // manual
    case 0x22:  
      LCD_Write_TextButton(9, 40, Manual, 0);
      return DataManual;

    // setup
    case 0x23:  
      LCD_Write_TextButton(9, 80, Setup, 0);
      return DataSetup;

    // sonic
    case 0x24:  
      LCD_Write_TextButton(9, 120, Sonic, 0);
      return DataSonic;
