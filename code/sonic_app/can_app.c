...
/* ------------------------------------------------------------------*
 *            TXMOB ACK Distance - One Shot
 * ------------------------------------------------------------------*/

void CAN_TXMOB_ACK_OneShot(void)
{
  unsigned char object[] = {1, 0x00, _oneShot};
  CAN_TXMOB(object);
}


/* ------------------------------------------------------------------*
 *            TXMOB ACK Distance - 5 Shots
 * ------------------------------------------------------------------*/

void CAN_TXMOB_ACK_5Shots(void)
{
  unsigned char object[] = {1, 0x00, _5Shots};
  CAN_TXMOB(object);
}
    ...
    if(object[1])
    {
      ...
      switch(object[2])
      {
        ...
        case _oneShot:
          if(state == _wait)
          {
            CAN_TXMOB_ACK_OneShot();
            return _oneShot;
          }
          else CAN_TXMOB_Working();
          break;

        case _5Shots:
          if(state == _wait)
          {
            CAN_TXMOB_ACK_5Shots();
            Sonic_5Shots(_init);
            return _5Shots;
          }
          else CAN_TXMOB_Working();
